...
问题：这样的继承方式会带来一个问题，`instance.constructor`变成了`Father`；为什么会造成这样的问题也是很显然的。`Father`的`prototype`是`Father`的原型对象，`Father`的原型对象的`constructor`是`Father`，`Son`的原型对象是`Father`的实例，也就是说`Son`的原型对象的`constructor`是`Father`，`instance`是`Son`的实例，自然`instance`的`constructor`也就指向`Father`了。
