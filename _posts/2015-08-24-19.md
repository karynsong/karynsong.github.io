...
事件循环：`node`在进程启动时就会建立一个循环，在一个时间周期内进行事件循环。时间周期称之为`Tick`，如果有事件待处理就取出事件及相关的回调函数。

观察者：怎么判断有没有待处理的事件呢，就是向观察者进行询问。观察者将事件收集到一起。等待被询问。谁来询问呢？网络请求、异步`I/O`等事件，他们能触发事件，触发事件之后告诉观察者。

请求对象：回调函数不是由开发者来调用的，是写好的之后内存来调用的。所以中间肯定还有事情发生，中间对象用于保存所有状态，事件的状态，事件的回调函数，在要执行回调函数的时候将这个回调函数推入线程池中。事件的回调被执行后也会存在这个里面。

执行回调：`I/O`线程池执行回调，将执行的回调再通过事件循环假如`I/O`观察者队列中。
...
`流程控制库`：主要是通过[async](http://karynsong.github.io/2015-09/28/)库来解决
